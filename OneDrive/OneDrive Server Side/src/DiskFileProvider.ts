...
export default class DiskFileProvider {
  ...
  isFolder(path: Path, isAbsolute: boolean = false): boolean {
    return fs
      .lstatSync(isAbsolute ? path.value : this._toAbsolute(path))
      .isDirectory();
  }
  ...
  write(path: Path, data: string | Buffer, isAbsolute: boolean = false) {
    ...
    if (this.exists(path)) fs.writeFileSync(absolutePath, data, "utf-8");
      ...
      fs.mkdirSync(this._toAbsolute(path.parentDirectoryPath), {
        recursive: true,
      });
  ...
  read(path: Path, isAbsolute: boolean = false): string {
    if (this.exists(path, isAbsolute))
      return fs.readFileSync(
        isAbsolute ? path.value : this._toAbsolute(path),
        "utf-8"
      );
    return null;
  }
  ...
  createWriteStream(path: string): fs.WriteStream {
    const directoryPath = new Path(path).parentDirectoryPath.toString();
  ...
  private _deleteFile(path: Path, isAbsolute: boolean = false) {
    if (this.exists(path, isAbsolute))
      fs.unlinkSync(isAbsolute ? path.value : this._toAbsolute(path));
  }
  ...
  private _isEmptyFolder(path: Path): boolean {
    if (this.exists(path))
      return fs.readdirSync(this._toAbsolute(path)).length === 0;
  }
